...
## 阶段 10：Skills + MCP：如何让 Skill 指导 MCP 工具调用

**状态**：已完成

**为什么值得做**

MCP 负责暴露外部工具，Skill 负责规定什么时候用、参数从哪里来、结果如何处理。这个阶段先验证最小正常链路：Skill 在正文中说明应该使用哪个 MCP Server 的哪个 Tool，Host 再把 MCP Tool schema 绑定给模型并执行真实调用。

**学习目标**

- 理解 Skill 与 MCP 的协同方式。
- 用 Skill 指导模型调用 MCP 工具。
- 明确 Skill、Host、模型和 MCP Server 在正常工具调用中的职责边界。

**核心概念**

- MCP 是外部能力接口；Skill 是使用这些接口的操作手册。
- Action Skill 可以规定工具调用顺序、参数来源和结果处理方式。
- Host 负责发现 MCP Tool、绑定 Tool schema 给模型，并执行模型提出的 Tool Call。

**实验内容**

- 编写一个订单查询 Skill，在正文中说明 `stage10-order-query` MCP Server 提供 `get_order` 工具。
- 编写一个最小 MCP Server，只暴露 `get_order(order_id)`。
- 编写一个线性实验版 Host，使用本地 Ollama `qwen3-coder:30b` 完成 Skill 路由、Tool schema 绑定、MCP 调用和最终回答。

**产出物**

- `labs/skills/foundations/examples/stage10-mcp-skill/`：Skill 指导 MCP 调用的最小实验代码。
- `labs/skills/foundations/10 | Skills + MCP：如何让 Skill 指导 MCP 工具调用.md`：说明 Skill、Host、模型和 MCP Server 各自负责什么。
...
- `labs/skills/foundations/examples/stage11-langgraph-skills/`：LangGraph + Skills runtime demo。
- `labs/skills/foundations/11 | Skills + LangGraph：如何把路由、执行和人工确认放进状态图.md`：说明 Skill 路由、MCP 工具规划、人工确认和 checkpoint 如何协作。
- `drafts/skills/11 | Skill、MCP和LangGraph放在一起如何协作.md`：从通用Agent任务出发，说明 Skill、MCP 和 LangGraph 如何共同完成一次可暂停、可恢复的执行流程。
