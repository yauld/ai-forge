...
from .adapters import DEFAULT_MCP_ENDPOINT, DEFAULT_OLLAMA_MODEL
from .adapters import REPO_ROOT, build_bound_model, build_model_tools, load_amap_mcp_url
from .agent import ALLOWED_TOOL_NAMES, DEFAULT_QUESTION
from .agent import build_citywalk_graph, build_initial_state


DEFAULT_GRAPHVIZ_OUTPUT = (
    REPO_ROOT / "labs/langgraph/foundations/assets/amap_citywalk_agent_graph.png"
)


def render_graphviz(output_path: Path) -> None:
    """使用 Graphviz 把当前 LangGraph 结构导出成 PNG。"""
    graph = build_citywalk_graph()
...
async def run_experiment(question: str) -> None:
    """连接高德 MCP，运行模型驱动的 LangGraph Agent loop，并打印 trace。"""
    ...
    async with streamable_http_client(mcp_url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            graph = build_citywalk_graph(model, session, allowed_tools)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="运行 LangGraph + 高德 MCP 的模型驱动 CityWalk Agent loop。"
    )
