...
| 05 | `MCP Transport：stdio 与 Streamable HTTP 如何传递消息.md` | 同一组消息如何通过 stdio 和 Streamable HTTP 传递？ | 进行中 |
...
完整操作步骤见 [examples/README.md](examples/README.md)。
...
如果只想建立概念直觉，可以先读前三篇；如果想观察实际消息流，再打开第四篇 Notebook。

## 设计要点

- Resource 适合暴露上下文，例如数据库 schema、业务指标口径和可寻址的订单详情。
- Tool 适合执行动作，例如按日期查询订单汇总或按状态筛选订单。
- Prompt 适合沉淀可复用任务模板，例如生成订单日报分析提示词。
- Server 必须在边界处做输入校验，不能假设 Host 或模型永远传入正确参数。
- Tool annotations 可以为 Host 提供风险提示，但不能替代权限控制和业务校验。

## 下一步

读完 MCP 后，可以继续进入：

- [LangGraph 实战](../../langgraph/foundations)：把 MCP 能力放入可控工作流。
- [LangChain 实战](../../langchain/foundations)：理解工具调用和 Agent 的运行时控制。
