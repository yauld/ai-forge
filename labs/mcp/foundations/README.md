...
| 13 | [MCP 远程授权：从 401 到第一次受保护调用.md](13%20%7C%20MCP%20远程授权：从%20401%20到第一次受保护调用.md) | Bearer Token 如何决定 Client 能否调用远程 Server？ | 已完成 |
| 14 | [MCP 工具太多，会不会撑爆模型上下文.md](../../../drafts/mcp/14%20%7C%20MCP%20工具太多，会不会撑爆模型上下文.md) | MCP Server 工具很多时，模型上下文是否一定会膨胀？ | 已完成 |
| 15 | [MCP Client 实战：用高德天气 Tool 跑通真实远程服务.md](15%20%7C%20MCP%20Client%20实战：用高德天气%20Tool%20跑通真实远程服务.md) | 自研 AI 应用如何通过 Streamable HTTP 使用官方远程 MCP Tool？ | 已完成 |
...
| [remote_auth_server.py](examples/remote_auth_server.py) / [remote_auth_resource_server.py](examples/remote_auth_resource_server.py) / [remote_auth_client.py](examples/remote_auth_client.py) | 对比无 Token 得到 `401`，再通过登录、同意和授权码交换取得 Token 后调用受保护 Tool。 |
| [amap_mcp_tools_probe.py](examples/amap_mcp_tools_probe.py) | 连接高德官方 MCP Server，打印远程 `tools/list` 返回的工具目录和参数 Schema。 |
| [amap_mcp_agent_demo.py](examples/amap_mcp_agent_demo.py) | 用本地 Ollama 模型选择高德 MCP 天气 Tool，再由 Python Host 执行并生成最终回答。 |
