...
| 11 | [MCP 内容安全：Prompt Injection、审计与敏感信息.md](11%20%7C%20MCP%20内容安全：Prompt%20Injection、审计与敏感信息.md) | 如何防止不可信内容越权并保留脱敏审计？ | 已完成 |
...
公众号精简稿和后续内容规划记录在
[MCP 内容路线图](../../../drafts/mcp/mcp_roadmap.md)；本表仍只列用于学习和复现的完整实验成果。
...
| [content_security_server.py](examples/content_security_server.py) / [content_security_host.py](examples/content_security_host.py) | 验证 Prompt injection 防护与审计脱敏。 |
| [security_order_data.py](examples/security_order_data.py) | 为四组安全实验重置独立的订单样例数据库。 |
